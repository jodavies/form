...
these lines are interpreted exactly like the lines in the setup file.
Any \#: lines which appear later in the file are ignored, and a warning
is printed. The
...
sizes of buffers are given in bytes, unless specified oitherwise. A ``word''
is 4 bytes on 64bit machines.
...
starts with a relatively small size and will grow if needed, up to this
specified maximum size. If more space is needed, \FORM\ will 
...
terms that are either written or read. Its size will be at least twice 
MaxTermSize but when there is heavy use of expressions in the right hand 
side of definitions or substitution it may have to be considerably 
larger. It is hoped that in the future this parameter can be eliminated. 
...
indices). The size of this parameter is not coupled to any array space.}
...
recognize the underscore as a valid character.}

\leftvitem{4.0cm}{FunctionLevels\index{setup!functionlevels}\index{functionlevels}}
\rightvitem{12.6cm}{The maximum number of nested levels that may occur, when 
functions have functions in their arguments.}

\leftvitem{4.0cm}{HideSize\index{setup!hidesize}\index{hidesize}}
\rightvitem{12.6cm}{The size of the hide buffer, in bytes. By default, the size
of this buffer is set equal to scratchsize (see below). When making heavy use
of the Hide system, specifying a larger value here may improve performance.}
...
look for files if they are not found in the current directory. This 
...
in \FORM\@. The number should be given in words. The number size is always
limited by the maximum size of the terms (see MaxTermSize); it must be fit in
(MaxTermSize-2)/2 words, since a coefficient represents both a numerator and
a denominator. By default, this is set to this maximum possible value, however
when setting very large values for MaxTermSize it can be a good idea to set a
smaller value for this parameter, since various internal buffers scale with
this size.}
...
size significantly affects the size of many allocations, since various
buffers are required to be able to fit at least one term. Larger values also
place a heavier demand on the WorkSpace, which acts as a heap during execution
and MaxTermSize space may be reserved in advance of a term operation, before
\FORM\ knows what the actual final size of the term will be. Consequently,
the evaluation tree can not be very deep when WorkSpace/MaxTermSize is not
very large. Its absolute maximum size is $2^{31}-1$ words, or about 2G.}
...
can be active in a single matching of a pattern. Under normal circumstances
...
\rightvitem{12.6cm}{The size, in bytes, of the input and the output buffers for the 
regular algebra processing. Expressions are read in in chunks this size and are 
...
\rightvitem{12.6cm}{Only relevant for \TFORM\@. This controls the size of batches
of terms which are sent to the workers. Each bucket will be at least as large as
MaxTermSize, but by default and for not-too-large MaxTermSize values they can hold
more than one term.
...
\rightvitem{12.6cm}{The size, in words, of the heap that is used by the algebra 
...
sorting inside \FORM\ a little bit before using them. In addition,
these parameters can noticably influence the performance of a computation.
See also chapter \ref{sorting} for more details.
...
buffer\index{buffer!small}''. Its size is controlled by the variable {\sl 
SmallSize\index{smallsize}}. When this buffer is full, or when the number 
of terms in this buffer exceeds a given maximum (controlled by the variable 
...
small buffer resides in the physical memory of the machine. During the sorting it may 
happen that term coefficient are added. The sum of two rational numbers (or
rational polynomials, in case PolyRatFun is active) can 
require more space than either of the individual numbers (or polynomials),
so there will be a 
...
controls the size of the small buffer together with this extension. The value for 
SmallExtension will always be at least 3/2 times the value of SmallSize,
but some computations may require it to be further increased.
...
LargeSize\index{largesize}}) as a single ``patch'' and the filling of the 
...
until no more terms are generated. In the end it will be necessary to sort 
...
limitation is that each cache should be able to contain at least two 
...
the variable FilePatches may be adjusted by \FORM\@, and a warning is printed.
If FilePatches reaches its minimal value of 2, LargeSize may be increased
to satisfy the above condition. Again, a warning is printed.
If there are more than 
FilePatches patches in the sort file, a second sort file is needed for the 
output of each sorted ``superpatch''\index{superpatch}. When the first sort file has 
...
buffer before it is sorted. The sorted result is copied to the large buffer
as a ``large patch''.}
...
\rightvitem{13cm}{The size of the small buffer plus its extension, in bytes.}

\leftvitem{3cm}{LargeSize\index{setup!largesize}\index{largesize}}
\rightvitem{13cm}{The size of the large buffer, in bytes.}
...
far more conservative here than with the global allocations.
...
meanings are the same as for the variables without the ``sub'' prefix.
...
the buffers of the master thread. The workers each allocate sorting buffers
that are 1/N times the size of the buffer of the master. These may be increased
in size to satisfy constraints, particularly for large values of MaxTermSize.
...
One may specify setup values using the suffixes ``K'', ``M'', ``G'', ``T'' to
indicate kilo-, mega-, giga- and tera-units, respecitively. For example, a
LargeSize of 1500000000 maybe be specified as 1500M.

The default values for settings related to ground-level sorting and expression
input/output buffers are:
\begin{center}
\begin{tabular}{lrr}
Variable         &      64-bit FORM  & 64-bit TFORM   \\ \hline
termsinsmall &          2M          & 3M           \\
smallsize (bytes) &     150M        & 300M         \\
smallextension (bytes)& 225M        & 450M         \\ \hline
largesize (bytes) &     750M        & 1500M        \\
largepatches &          256         & 256          \\
filepatches &           256         & 256          \\ \hline
hidesize (bytes) &      500M        & 500M         \\
scratchsize (bytes) &   500M        & 500M         \\
sortiosize (bytes) &    200K        & 200K         \\ \hline
threadscratchoutsize (bytes) & $-$  & 2500K        \\
threadscratchsize (bytes) & $-$     & 160K
\end{tabular}
\end{center}

The default values for settings related to sub-buffer sorting, e.g.~inside
function arguments, dollar variables and term environments, are:
\begin{center}
\begin{tabular}{lr}
Variable         &      64-bit FORM and TFORM  \\ \hline
subtermsinsmall &          30K        \\
subsmallsize (bytes)&      3000K      \\
subsmallextension (bytes)& 4500K      \\ \hline
sublargesize (bytes)&      15M        \\
sublargepatches &          64         \\
subfilepatches &           40         \\ \hline
subsortiosize (bytes)&     160K       \\
\end{tabular}
\end{center}

The default values for other settings are:
\begin{center}
\begin{tabular}{lr}
Variable         &      64-bit FORM and TFORM \\ \hline
bracketindexsize (bytes) & 3200K        \\
commentchar &              $*$          \\
compresssize (bytes)&      320K         \\
constindex &               128          \\
...
incdir &                   .            \\
insidefirst &              ON           \\
maxnumbersize (words) &    Max.~for given maxtermsize \\
maxtermsize (words) &      40K          \\
maxwildcards &             100          \\
nospacesinnumbers &        OFF          \\
numstorecaches &           4            \\
nwritefinalstatistics &    OFF          \\
nwritestatistics &         OFF          \\
nwritethreadstatistics &   OFF          \\
oldorder &                 OFF          \\
parentheses &              100          \\
path &                     .            \\
processbucketsize &        1000         \\
...
To find out what the current setup values are, one can use the `ON,setup;' 
