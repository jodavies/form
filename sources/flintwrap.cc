...
void flint_poly_from_argument(fmpz_mpoly_t, fmpz_mpoly_t, const WORD *, const map<unsigned,unsigned>, fmpz_mpoly_ctx_t);
ULONG flint_poly_to_argument(WORD *, fmpz_mpoly_t, const map<unsigned,unsigned>, fmpz_mpoly_ctx_t);
...
*/
// TODO sort vars
map<unsigned,unsigned> flint_get_variables(const vector <WORD *> &es, bool with_arghead, bool sort_vars) {
	DUMMYUSE(sort_vars);
...
*/
// TODO with arghead
void flint_poly_from_argument(fmpz_mpoly_t poly, fmpz_mpoly_t denpoly, const WORD *args, const map<unsigned,unsigned> var_map, fmpz_mpoly_ctx_t ctx) {
	...
	for (const WORD* term = args; term < arg_stop; term += term[0]) {
		...
		if (*(symbol_stop+ABS(coeff_size/2)) != 1) {
			fmpz_t tmp;
			fmpz_init(tmp);
			flint_fmpz_set_form(tmp, (UWORD*)(symbol_stop+ABS(coeff_size/2)), coeff_size/2);
			// Record the LCM of the coefficient denominators:
			fmpz_lcm(den_coeff, den_coeff, tmp);
			fmpz_clear(tmp);
		}
	}
	// Assemble denpoly. Flip the signs
	...
	for (const WORD* term = args; term < arg_stop; term += term[0]) {
		...
		if (t == symbol_stop) {
			...
			t++; // this entry just has the size of the symbol array, but we can use symbol_stop
			for (const WORD* s = t; s < symbol_stop; s+=2) {
				exponents[var_map.at(*s)] = *(s+1) + neg_exponents[var_map.at(*s)];
			}
		}
		...
		if (*(symbol_stop+ABS(coeff_size/2)) != 1) {
			...
			fmpz_init(tmp);
			flint_fmpz_set_form(tmp, (UWORD*)(symbol_stop+ABS(coeff_size/2)), coeff_size/2);
			fmpz_divexact(coeff, coeff, den_coeff);
			fmpz_clear(tmp);
		}
...
*/
// TODO with arghead
ULONG flint_poly_to_argument(WORD *out, fmpz_mpoly_t poly, const map<unsigned,unsigned> var_map, fmpz_mpoly_ctx_t ctx) {
...
//		cout << "   term: ";
//		fmpz_print(coeff);
//		cout << " : " << exponents[0] << " " << exponents[1] << endl;
		int size = flint_fmpz_get_form(coeff, out);
//		cout << "size: " << size << endl;
		out += ABS(size);
		...
		*out = 2*ABS(size) + 1; // the size of the coefficient
		if ( size < 0 ) { *out = -(*out); }
		out++;
...
//	const bool clean = (a[2] & MUSTCLEANPRF) == 0;
	...
	// Read the numerator
	flint_poly_from_argument(num, den_num, a, var_map, ctx);

	NEXTARG(a);
	if ( a < arg_stop ) {
		// Read the denominator
		flint_poly_from_argument(den, den_den, a, var_map, ctx);
		NEXTARG(a);
	}
	else {
