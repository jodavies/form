...
*/
			if ( ModOptdollars[j].type == MODMAX || ModOptdollars[j].type == MODMIN ) {
				...
				for ( i = 0; i < AM.totalnumberofthreads; i++ ) {
					...
					// the term data should be "4 N 1 +-3". We need to take the appropriate value
					// and compare it with the global data. We have to take care in the
					// comparisons, since the numbers are stored as WORD not UWORD.
